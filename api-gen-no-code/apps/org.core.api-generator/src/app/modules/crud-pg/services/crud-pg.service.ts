...
import {
  QueryParamDataDto,
  RequestParamDataDto,
} from '../controller/query-filter.dto';
import {
  ConditionObject,
  JoinTable,
} from '../../../core/pgsql/pg.relationaldb.query-builder';
...
export class CrudService {
  ...
  public async getColumnsToResonseEachAPI(table: string, action: string) {
    ...
    const tableInfo = await this.queryBus.execute(
      new GetSchemaStructureQuery(appInfo, appid, '_core_generated_apis')
    );
    ...
    const queryResult = await this.queryBus.execute(
      new GetDataQuery(
        ...
        {
          and: [{ table_name: table }, { action: action }],
        }
      )
    );

    const availableColumns = queryResult[0]?.api_authorized?.columns
      ?.filter((el) => {
    ...
    console.log(
      `Available colunms of table ${table} with action ${action} are: ${JSON.stringify(
        availableColumns
      )}`
    );
  ...
  private async getApplicationInfo(
    ...
    const workspaceConnection = await this.queryBus.execute(
      new GetWorkspaceConnectionQuery()
    );
    return this.queryBus.execute(
      new GetAppInfoByAppId(workspaceConnection, appId)
    );
  }
  ...
  async insert(appId: string, schema: string, data: Array<object>) {
    const appInfo = await this.getApplicationInfo(appId);
    const tableInfo = await this.queryBus.execute(
      new GetSchemaStructureQuery(appInfo, appId, schema)
    );
    const returningQueryByTable = await this.getColumnsToResonseEachAPI(
      schema,
      'INSERT'
    );
  ...
  async delete(appId: string, schema: string, id: number, column: string) {
    ...
    return this.commandBus.execute(
      new DeleteDataCommand(appInfo, tableInfo, appId, schema, id, column)
    );
  }
